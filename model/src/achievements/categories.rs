...
// I would consider this nomenclature asinine if it weren't literally how it was
// documented.
...
pub struct AchievementDetails {
    ...
    /// The inclusive level range for a daily achievement. Available on
    /// achievements that are level locked.
    level: Option<[u8; 2]>,
}
...
pub struct AchievementCategory {
    ...
    /// categories in its group. Lowest numbers go first, highest numbers go
    /// last.
    ...
    /// A list of achievements that will be active tomorrow. This is only
    /// available on some daily categories.
